...
keithley_gui = KeithleyGuiApp(Source_meter)
keithley_gui.show()
app.exec()
...
#Setting up Source meter functions
...
def bjt_vsweep(vbestart, vbestop, vcebias):
    ...
    l_vbestep = (vbestop - vbestart) / (l_vbesteps - 1)
    l_vbesource_val = vbestart
    l_vbe_i = 1
...
def mosfet_vsweep(vgsstart, vgsstop, vdsstart, vdsstop):
    ...
    l_id_data = []
    l_vgs_data = []
    #data={'Current' : l_id_data, 'Vds' : l_vds_data, 'Vgs' : l_vgs_data}
    IV_df=pd.dataframe([], index = [], col = [])
    fakeData=pd.dataframe(pd.read_csv('fakeData.csv'), index=[0], columns=['current','voltage'])
    #  Execute sweep
    for l_vgs_iteration in range(1, l_vgssteps):
        ...
        for l_vds_iteration in range(1, l_vdssteps):
            l_vgs_data[l_vgs_iteration] = fakeData[0]  # Measure gate-source voltage
            # l_vgs_data[l_vgs_iteration] = Source_meter.smub.measure.v()  # Measure gate-source voltage
            if l_vds_iteration == 1:
                l_vdssource_val = vdsstart

            l_vds_data[l_vgs_iteration][l_vds_iteration] = fakeData['voltage']  # Measure sourced voltage
            l_id_data[l_vgs_iteration][l_vds_iteration] = fakeData['current']  # Measure current
        ...
        l_vgssource_val += l_vgsstep  # Calculate new source value
        data={'Current' : l_id_data, 'Vds' : l_vds_data, 'Vgs' : l_vgs_data}
        IV_df[('Current', 'Vds', 'Vgs')]= data
        print(IV_df)
    ...
    # making a new table for every drain sweep at a new gate voltage.
